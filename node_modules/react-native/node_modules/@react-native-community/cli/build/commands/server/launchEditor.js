...
function launchEditor(fileName, lineNumber, projectRoots) {
  ...
  if (process.platform === 'win32' && !WINDOWS_FILE_NAME_WHITELIST.test(fileName.trim())) {
    _cliTools().logger.error(`Could not open ${_path().default.basename(fileName)} in the editor.`);

    _cliTools().logger.info('When running on Windows, file names are checked against a whitelist ' + 'to protect against remote code execution attacks. File names may ' + 'consist only of alphanumeric characters (all languages), periods, ' + 'dashes, slashes, and underscores.');

    return;
  }

  _cliTools().logger.info(`Opening ${_chalk().default.underline(fileName)} with ${_chalk().default.bold(editor)}`);
  ...
  _childProcess.on('exit', errorCode => {
    ...
    if (errorCode) {
      _cliTools().logger.error('Your editor exited with an error!');
